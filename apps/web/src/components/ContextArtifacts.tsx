...
export function ContextArtifacts({config,projectId,onSelect,onArtifact}:{config:ApiClientConfig;projectId:string;onSelect:(id:string)=>void;onArtifact:(id:string)=>void}) {
  ...
  return <section className="context-artifacts" aria-label="Context versions and artifacts"><h3>Context and immutable artifacts</h3>
    ...
    <label>Upload artifact<input type="file" disabled={busy} onChange={e=>{const file=e.target.files?.[0];if(file)void upload(file)}}/></label>
    <p>Uploaded content is immutable and retained. Add its reference to a new context version.</p>
    <ul>{artifacts.map(a=><li key={a.id}><button type="button" onClick={()=>onArtifact(a.id)}>Add artifact {a.id}</button> · {a.size} bytes · {a.sha256}</li>)}</ul>
  </section>
}
