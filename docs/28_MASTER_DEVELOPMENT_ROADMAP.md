> 2026-09-10 Track A navigation override — 以下CP/Gxx順序、進度分數與日期為HISTORICAL / NOT_ACTIVE_ROUTING。所有legacy項目只由reconciliation映射至Track A，不再形成第二套active roadmap。
>
...
### State/provenance reconciliation delta — 2026-09-01

- Primary checkout remains `feature/first-vertical-slice@b87a0dc`, dirty with
  staged state empty and upstream `backup/feature/first-vertical-slice`.
- A later isolated candidate is clean at
  `730912b5a3e19449c355975485f1fe77350a458a`, ahead 22 of its local-Temp
  `origin/feature/first-vertical-slice`. Its commit range includes G13–G18,
  CP04 WP17–WP20, CP05, and CP06 nonbrowser work, with WP21 explicitly deferred.
- Candidate status and provisional RC claims are now promotion-backed at
  approved backup `feature/first-vertical-slice@730912b`; exact-SHA clean-clone
  and fresh Core/extension/validator evidence passed. They do not yet earn
  formal roadmap points or convert CP04–CP06 to accepted until the required
  Human checkpoint acceptance is recorded; WP21 remains deferred/unverified.
- This delta changes no checkpoint weight, scope boundary, ADR, or acceptance
  rule. It records two lanes so candidate claims cannot be mistaken for primary
  acceptance or cross-machine readiness.
...
### Current completion status reconciliation — 2026-09-01

The candidate lane reports CP-03 work through G18 complete, CP04 WP17–WP20
complete with WP21 deferred/unverified, CP05 complete, and CP06 nonbrowser work
complete with provisional RC acceptance. The primary lane remains CP-03
`IN_PROGRESS`, CP04–CP06 `PLANNED`, with formal project progress **30/100**.
Candidate-only completion earns no roadmap points until the candidate has an
approved remote checkpoint, exact-SHA clean-clone verification, and fresh
acceptance from that checkpoint.
