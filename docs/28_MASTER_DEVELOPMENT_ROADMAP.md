...
- Next ordered gates: independently reviewed Governance checkpoint including explicitly approved D11 -> separately authorized curated ADR-011 docs-only import -> Pre-WP14-A ADR-007 lifecycle/cleanup gate -> Pre-WP14-B FULL ADR-011 Runtime Binding/migration architecture gate -> separately authorized OpenCode implementation with fresh independent Codex review.
...
| PRE-WP14-A | ADR-007 timeout, cancel, lifecycle and cleanup architecture/conformance gate | OpenCode / Codex | Gated | Before WP-14 | Human-approved existing-contract compatibility; deterministic cleanup/failure-isolation plan; no premature runtime implementation |
| PRE-WP14-B | FULL ADR-011 Runtime Binding architecture, Run-owned immutable snapshot, Alembic migration/backfill/rollback plan | OpenCode / Codex | Gated | Before WP-14 | Curated ADR document integration, Human Architecture Gate, approved source/migration allowlist and deterministic legacy/restore tests; no Attempt or new subsystem |
