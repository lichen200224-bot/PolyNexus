...
| OBJECTIVE | 只按 F2 批准 diff 同步正式文件 |
| WHY | 消除 FORMALIZATION 前置缺口，形成可獨立審查的 bounded checkpoint，不把 WP 當巨大 coding prompt |
| CURRENT_STATE | 正式同步 NOT YET EXECUTED |
| TARGET_STATE | 正式文件與 approved intent 一致，maturity 不變 |
| INPUT | Freeze Record / REV1 / Work Packages / Formal Change Plan；canonical design/read f34e6b29ae9e7326d1d44b9b03756b450809928f |
| PREDECESSOR_CHECKPOINT | UNASSIGNED；TA-F2 explicit wording and file authorization + safe lane；授權前填 accepted exact SHA/remote/decision refs，不能用設計SHA冒充 |
| IN_SCOPE | 只按 F2 批准 diff 同步正式文件 |
...
| EXPECTED_FILE_AREAS | F2 exact allowlist；Decision/ADR → SA/SD → PRD/Scope refs，非整檔覆蓋 |
| FROZEN_INVARIANTS | 全部適用；重點 I-01 through I-23；原文 authority為Freeze Record §3與REV1 §24 |
| DEPENDENCIES | TA-F2 explicit wording and file authorization + safe lane |
...
| POSITIVE_TESTS | actual approved diff 一致、links/invariants、前後hash、歷史與D11-C保留 |
| NEGATIVE_TESTS | 任何超出 wording/allowlist 或 source drift HOLD；不修改產品、不把未實作改完成 |
...
| NEXT_GOAL | TA-F4；需其本身授權，不從NEXT推permission |
