# TA-F4 — Independent documentation consistency acceptance
...
| OBJECTIVE | Independent documentation consistency acceptance |
| WHY | 消除 FORMALIZATION 前置缺口，形成可獨立審查的 bounded checkpoint，不把 WP 當巨大 coding prompt |
| CURRENT_STATE | F3 candidate 未審 |
| TARGET_STATE | 獨立 verdict 與 Human checkpoint decision；S0仍需授權 |
| INPUT | Freeze Record / REV1 / Work Packages / Formal Change Plan；canonical design/read f34e6b29ae9e7326d1d44b9b03756b450809928f |
| PREDECESSOR_CHECKPOINT | UNASSIGNED；TA-F3 exact review candidate + designated non-writer reviewer；授權前填 accepted exact SHA/remote/decision refs，不能用設計SHA冒充 |
| IN_SCOPE | Independent documentation consistency acceptance |
...
| EXPECTED_FILE_AREAS | read-only exact F3 SHA + independent review record；不修文件 |
| FROZEN_INVARIANTS | 全部適用；重點 I-01 through I-23；原文 authority為Freeze Record §3與REV1 §24 |
| DEPENDENCIES | TA-F3 exact review candidate + designated non-writer reviewer |
| INTEGRATION_DEPENDENCIES | sequential accepted predecessor；若批准平行拆分，另需accepted integration SHA |
| GOAL_OWNER | Human-designated independent reviewer |
| PREFERRED_IMPLEMENTATION_WRITER | CODEX；本Goal read-only，F3 Writer不可自審 |
...
| POSITIVE_TESTS | 核對 I-01..I-23、crossrefs、D11-C、legacy/migration、P0/N1、actual exits |
| NEGATIVE_TESTS | missing approval、scope drift、未實作標PASS、改golden或public runtime語意均阻擋 |
...
| REVIEW_CANDIDATE_REQUIREMENT | 只review exact F3 candidate SHA；不可邊修邊審 |
| COMPLETION_CRITERIA | 本contract正負criteria全部有fresh evidence，required skip不可PASS；scope/invariant自查；獨立verdict與Human checkpoint decision；不宣稱Working Product完成 |
...
| NEXT_GOAL | TA-S0-G01；需其本身授權，不從NEXT推permission |
