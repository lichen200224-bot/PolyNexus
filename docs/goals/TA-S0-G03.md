# TA-S0-G03 — UI/Core pairing foundation、START與分層HEALTH
...
| OBJECTIVE | UI/Core pairing foundation、START與分層HEALTH |
| WHY | 消除 S0 前置缺口，形成可獨立審查的 bounded checkpoint，不把 WP 當巨大 coding prompt |
| CURRENT_STATE | client/startup基礎待驗 |
| TARGET_STATE | fresh client可連接，schema/Core/Web/executor readiness分開 |
| INPUT | Freeze Record / REV1 / Work Packages / Formal Change Plan；canonical design/read f34e6b29ae9e7326d1d44b9b03756b450809928f |
| PREDECESSOR_CHECKPOINT | UNASSIGNED；TA-S0-G02 accepted；新增Human grant/session前 formal M-HUMAN approved；授權前填 accepted exact SHA/remote/decision refs，不能用設計SHA冒充 |
| IN_SCOPE | UI/Core pairing foundation、START與分層HEALTH |
...
| EXPECTED_FILE_AREAS | api/dependencies.py、api/health.py、app.py、Web client/App、start_core/start_web及同責任start_dev與tests；不新增Accept通道 |
| FROZEN_INVARIANTS | 全部適用；重點 I-01/12/13/15/17/20/21/23；原文 authority為Freeze Record §3與REV1 §24 |
| DEPENDENCIES | TA-S0-G02 accepted；新增Human grant/session前 formal M-HUMAN approved |
...
| POSITIVE_TESTS | START→health→paired API成功；S0-G01/G02 accumulated regression；獨立browser journey |
| NEGATIVE_TESTS | missing/wrong token拒絕；port collision/schema非head/missing service不可ready；pairing不授予Agent Human Accept |
...
| NEXT_GOAL | TA-W1-G01；需其本身授權，不從NEXT推permission |
