# TA-W1-G03 — generation-level writer lineage與Retry/late Abort安全
...
| OBJECTIVE | generation-level writer lineage與Retry/late Abort安全 |
| WHY | 消除 W1 前置缺口，形成可獨立審查的 bounded checkpoint，不把 WP 當巨大 coding prompt |
| CURRENT_STATE | 只有一般claim/idempotency仍不足 |
| TARGET_STATE | 同generation僅一條合法lineage，late abort不影響新generation |
| INPUT | Freeze Record / REV1 / Work Packages / Formal Change Plan；canonical design/read f34e6b29ae9e7326d1d44b9b03756b450809928f |
| PREDECESSOR_CHECKPOINT | UNASSIGNED；TA-W1-G02 accepted checkpoint；授權前填 accepted exact SHA/remote/decision refs，不能用設計SHA冒充 |
| IN_SCOPE | generation-level writer lineage與Retry/late Abort安全 |
...
| EXPECTED_FILE_AREAS | execution_service.py、api/runs.py、workspace claim/CAS與targeted concurrency/integration tests |
...
| DEPENDENCIES | TA-W1-G02 accepted checkpoint |
...
| POSITIVE_TESTS | g1/R1安全釋放→g2/R2；同command重送receipt；W1完整regression；實際launch副作用計數 |
| NEGATIVE_TESTS | different command_id/不同入口/並發Create-Start/多未claimed intent僅一lineage；釋放後不能 unrelated second lineage；late Abort(g1)不改g2/R2/control；aborted不launch |
...
| NEXT_GOAL | TA-W2；需其本身授權，不從NEXT推permission |
