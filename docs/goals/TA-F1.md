# TA-F1 — 準備 M-IDENTITY/M-HUMAN exact diff proposal，不套用正式檔案
...
| OBJECTIVE | 準備 M-IDENTITY/M-HUMAN exact diff proposal，不套用正式檔案 |
...
| POSITIVE_TESTS | 逐段對應 accepted REV1 / freeze / Formal Change Plan；保存 base file hashes、compatibility/migration/test/rollback impact |
| NEGATIVE_TESTS | proposal 不改正式檔；舊 dirty 段落不被覆蓋；M-EXECUTION 未證明必要不標 REQUIRED |
...
| NEXT_GOAL | TA-F2；需其本身授權，不從NEXT推permission |
