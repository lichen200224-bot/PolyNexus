"""OpenCode Runtime Adapter (WP-15).

Deterministic, no-network conformance adapter for the OpenCode runtime
identity. All vendor knowledge lives inside this adapter; Core contains no
...
_NORMALIZED_FAILURE_ERROR = "OpenCode runtime reported execution failure"

_UNKNOWN_REF_ERROR = "Unknown OpenCode runtime reference"

_NO_RESUME_ERROR = "OpenCode runtime has no resume capability"

_RESULT_SUMMARY = "OpenCode runtime completed deterministic local execution"
...
class OpenCodeRuntimeAdapter:
    """Deterministic local OpenCode runtime adapter satisfying RuntimeAdapter."""

    def __init__(self) -> None:
        self._runs: dict[str, _OpenCodeRun] = {}
    ...
    def capabilities(self) -> RuntimeCapabilities:
        ...
        # WP-15 targeted evidence; no usage data and no managed credentials.
    ...
    async def create_run(self, context: ContextPackage) -> str:
        runtime_ref = f"opencode:{uuid4().hex}"
        self._runs[runtime_ref] = _OpenCodeRun(context=context)
        return runtime_ref

    async def submit(self, runtime_ref: str, task: Task) -> None:
        record = self._get(runtime_ref)
        if record.state is not RunState.CREATED:
            raise ValueError(f"OpenCode runtime cannot submit from state {record.state.value}")
        record.task = task
    ...
    async def result(self, runtime_ref: str) -> RuntimeResult:
        ...
        if record.state not in {RunState.RUNNING, RunState.COMPLETED}:
            raise ValueError(f"OpenCode runtime has no result in state {record.state.value}")
    ...
    def version_info(self) -> str:
        return "opencode-local-adapter/0.1"
    ...
    def simulate_timeout(self, runtime_ref: str) -> None:
        ...
        if record.state is not RunState.RUNNING:
            raise ValueError(f"OpenCode runtime cannot time out from state {record.state.value}")
    ...
    def simulate_failure(self, runtime_ref: str) -> None:
        ...
        record = self._get(runtime_ref)
        if record.state is not RunState.RUNNING:
            raise ValueError(f"OpenCode runtime cannot fail from state {record.state.value}")
        record.state = RunState.FAILED

    def _get(self, runtime_ref: str) -> _OpenCodeRun:
